...
export class InvestorService {
  constructor(
    ...
    private investorRepository: Repository<InvestorProfile>,
  ) {}
  ...
  async trackProfileView(umkmId: string, investorId?: string, sessionId?: string) {
    // Track view in profile_views table
    // Increment profile_views counter
    await this.investorRepository.query(
      'UPDATE umkm_profiles SET profile_views = profile_views + 1 WHERE id = $1',
      [umkmId]
    );

    // Insert view record
    if (investorId || sessionId) {
      await this.investorRepository.query(
        'INSERT INTO profile_views (umkm_id, investor_id, session_id, viewed_at) VALUES ($1, $2, $3, NOW())',
        [umkmId, investorId, sessionId]
      );
    }
  }

  async addBookmark(investorId: string, umkmId: string, notes?: string) {
    await this.investorRepository.query(
  ...
  async removeBookmark(investorId: string, umkmId: string) {
    await this.investorRepository.query(
  ...
  async getBookmarks(investorId: string) {
    return this.investorRepository.query(
      `SELECT 
        u.*,
        b.notes,
